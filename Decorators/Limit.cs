...
namespace BehaviorTree
{
	...
	public class Limit<T> : Decorator<T>
	{
		private readonly TimeSpan maxRunTime;
		private readonly ITime time;
		private DateTime? endTime;

		public Limit(TimeSpan maxRunTime, Func<T, Result> node)
			: this(maxRunTime, new Node<T>(node)) { }

		public Limit(TimeSpan maxRunTime, INode<T> node)
			: this(maxRunTime, node, Time.Real)
		{ }

		internal Limit(TimeSpan maxRunTime, INode<T> node, ITime time) : base(node)
		{
			this.maxRunTime = maxRunTime;
			this.time = time;
		}
